...
public class WishlistController {
	...
	public String addProductDetailWishlist(@RequestParam("id") String itemId,@RequestParam("category") String category, HttpServletRequest req, @RequestParam Integer productId,String search,HttpSession session) {
		mDAO.isLogined(req, session);
		wDAO.addWishlist(req,productId);
		pDAO.getCategory(req);
		pDAO.search(req);
	...
	public String removeProductDetailWishlist(@RequestParam("id") String itemId,@RequestParam("category") String category, HttpServletRequest req, @RequestParam Integer productId,String search,HttpSession session) {
		mDAO.isLogined(req, session);
		if (wDAO.isProductWishlistdByUser(req, productId)) {
			wDAO.removeWishlist(req, productId);
		}
