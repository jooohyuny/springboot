...
import com.kim.aug17nb.townnews.rp.TownNewsReplyRepo;
...
public class TownNewsDAO {
	...
	@Autowired
	private TownNewsRepo tnr;
	
	@Autowired
	private TownNewsReplyRepo tnrr;
	
	// 게시판 등록 메소드
	public void regTownNews(TownNews tn, HttpServletRequest req) {
		try {
			tnr.save(tn); // 게시글 등록
	...
	public void getTownsNewsPage(int page, HttpServletRequest req) {
	    try {
	        ...
	        List<TownNews> townNews = tnr.findAll(pageAble);
	        List<TownNewsReply> townNewsReply = tnrr.findAll();
	...
	public void getTownNewsContents(HttpServletRequest req) {
	    try {
	        ...
	        Optional<TownNews> townNewsOptional = tnr.findById(townNewsNum);

	        if (townNewsOptional.isPresent()) {
	            TownNews townNews = townNewsOptional.get();
	...
	// 삭제메소드
	public void townNewsDelete(TownNews tn, HttpServletRequest req) {
		try {
